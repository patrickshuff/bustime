...
try:
    from google.appengine.api.urlfetch import fetch as uo
except:
...
def urlopen(url):
    data = uo(url)
    if uo.__name__ == 'fetch': return data.content
    else: return data.read()
...
class bustime (object):
    ...
    def getpredictions(self, stpid='', rt='',vid='', top='', **kwargs):
	    ...
	    pr['diff_minutes'] = relativedelta(parse(pr['prdtm']), parse(pr['tmstmp'])).minutes
	    pr['diff_text'] = 'Approaching' if pr['diff_minutes'] == 1 else "%s Minutes" % pr['diff_minutes']
        return predictions
